...
# library("devtools")
#install_github("sherresthal/DESeqtools", ref = "package")
```
...
## Boxplot of normalized and unnormalized gene expression
...
Sometimhes it can be helpful to plot the mean values of conditions in a heatmap. For this, specify the option *plot_mean = TRUE* and put the column you would like to mean over with *plot_mean_column = "condition"* (as an example)

### Heatmap on mean expression values per condition
...
# you can also specify a different column of your sample table: 
plotHeatmap(input=norm_anno,
            sample_annotation = sample_table,
            column_annotation = plot_annotation,
            geneset = "all",
...
plotHeatmap(input=norm_anno,
            ...
            show_rownames = FALSE,
            cluster_cols = TRUE,
            column_annotation = c("condition", "treatment"))
```
...
dds_vst_df<-as.data.frame(assay(dds_vst)) #Deniz

plotLoadings(input=dds_vst_df,PC="PC1", ntop="all")
plotLoadings(input=dds_vst_df,PC="PC2", ntop="all")
plotLoadings(input=dds_vst_df,PC="PC3", ntop="all")
```
...
### PCA loadings
```{r}
#Deniz: with batch correction
plotLoadings(input = removedbatch_dds_vst,PC="PC1", ntop="all")
plotLoadings(input = removedbatch_dds_vst,PC="PC2", ntop="all")
plotLoadings(input = removedbatch_dds_vst,PC="PC3", ntop="all")
```
