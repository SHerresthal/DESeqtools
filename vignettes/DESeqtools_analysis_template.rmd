...
The aim was to write a script that can be used as a template for standard bulk RNA-seq analysis. For this example analysis, the a RNASeq dataset of human isolated monoctyes, which were  differentiated to monocyte-derived dendritic cells (moDC) in presence or absence of interleukin-10 (IL-10). Additional cell maturation by application of LPS leads to a total number of 4 conditions with 3 samples each.The raw data is accessible under https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE92852. 
...
library("devtools")
install_github("SHerresthal/RNA-DESeq2", ref = "package")
```
...
```{r}
plotDEHeatmap("LPStreated_moDCs_vs_Immature_moDCs",
                factor="condition",
                conditions="all")
  
  plotDEHeatmap("Immature_IL10APCs_vs_Immature_moDCs",
                factor="condition",
